...
#sleep 2
timeout 60 bash -c 'until ( dc_count=$( docker exec -i -t Cassandra-1 bash -c "nodetool status" | grep -e "Datacenter" -c ) ; if [ $dc_count -eq 2 ]; then true; else false; fi ) >/dev/null 2>&1; do sleep 1; echo -n "."; done'
